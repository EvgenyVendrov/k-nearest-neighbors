...
def knn(num_of_rounds=500, path_for_data_set=r"dataset\HC_Body_Temperature.txt"):
    ...
    dict_of_accuracy_by_k_and_p = {}
    for i in range(0, num_of_rounds):
        (shuffled_train_set, shuffled_test_set) = data_prep.shuffle_dataset(data_set)
        for k in range(1, 10, 2):
            for p in range(1, 4):
                dict_of_accuracy_by_k_and_p["%dk_%dp_%d" % (k, p, i)] = calculations.get_acc_on_train_set(
                    shuffled_train_set,
                    shuffled_test_set,
                    k, p)
    list_of_acc_avg = calculations.get_avg_error_by_k_and_p(dict_of_accuracy_by_k_and_p)
    i = 0
    for k in range(1, 10, 2):
        for p in range(1, 4):
            print_avg(k, p, list_of_acc_avg[i])
            i += 1
